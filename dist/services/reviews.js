...
export function createReview(input) {
    ...
    const job = db.prepare('SELECT * FROM jobs WHERE id = ?').get(input.jobId);
    if (!job) {
    ...
    const existing = db.prepare('SELECT id FROM reviews WHERE job_id = ? AND reviewer_id = ?').get(input.jobId, input.reviewerId);
    if (existing) {
...
export function listReviews(options = {}) {
  ...
  `).all(...params, limit, offset);
    ...
    if (options.revieweeId && reviews.length > 0) {
        const avgResult = db.prepare('SELECT AVG(rating) as avg FROM reviews WHERE reviewee_id = ?').get(options.revieweeId);
