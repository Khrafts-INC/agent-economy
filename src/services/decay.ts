 ...
 */
function getLastJobCompletionDate(agentId: string): Date | null {
  const db = getDb();
  const row = db.prepare(`
    SELECT MAX(completed_at) as last_completed
    FROM jobs 
    WHERE provider_id = ? AND status = 'completed' AND completed_at IS NOT NULL
  `).get(agentId) as { last_completed: string | null } | undefined;
 ...
 */
function getDaysInactive(agentId: string, createdAt: Date): number {
 ...
 */
export function applyReputationDecay(): DecaySummary {
  const db = getDb();
    ...
    if (decay > 0) {
      ...
      if (actualDecay > 0) {
        ...
        `).run(
          uuid(),
          agent.id,
          `Reputation decay: ${previousScore.toFixed(2)} → ${newScore.toFixed(2)} (${daysInactive} days inactive)`,
          now
        );
 ...
 */
export function previewReputationDecay(): DecaySummary {
  const db = getDb();
  const now = new Date().toISOString();
 ...
 */
export function getAgentDecayStatus(agentId: string): {
  agentId: string;
  currentScore: number;
  daysInactive: number;
  pendingDecay: number;
  ...
  `).get(agentId) as { id: string; reputation_score: number; created_at: string } | undefined;
