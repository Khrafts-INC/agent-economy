...
type LeaderboardMetric = 'reputation' | 'jobs' | 'earnings';

const VALID_METRICS: LeaderboardMetric[] = ['reputation', 'jobs', 'earnings'];
...
app.get('/', (c) => {
  ...
  `).all() as Array<{ category: string; provider_count: number; completed_jobs: number }>;
...
app.get('/:category', (c) => {
  ...
  const metric = (c.req.query('metric') || 'reputation') as LeaderboardMetric;
  ...
  const leaders = db.prepare(`
    ...
    ORDER BY ${orderBy}
    LIMIT ?
  `).all(category, category, MIN_JOBS_FOR_LEADERBOARD, limit) as Array<{
    agent_id: string;
    name: string;
    reputation_score: number;
    total_jobs_completed: number;
    category_jobs_completed: number;
    category_earnings: number;
    value: number;
  }>;
  ...
  const qualifiedCount = db.prepare(`
    ...
    WHERE a.total_jobs_completed >= ?
  `).get(category, MIN_JOBS_FOR_LEADERBOARD) as { count: number };
...
app.get('/:category/me/:agentId', (c) => {
  ...
  const agent = db.prepare(`
    ...
    FROM agents a 
    WHERE a.id = ?
  `).get(category, agentId) as { 
    id: string; 
    name: string; 
    reputation_score: number; 
    total_jobs_completed: number;
    has_category_services: number;
  } | undefined;
  ...
  `).get(category, agentId, MIN_JOBS_FOR_LEADERBOARD, agentId) as { rank: number };
  ...
  `).get(agentId, category) as { jobs_completed: number; earnings: number };
