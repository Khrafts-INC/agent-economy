...
jobRoutes.post('/', async (c) => {
  try {
    ...
    if (!body.requesterId) return c.json({ error: 'requesterId is required' }, 400);
    ...
    if (error.message?.includes('Insufficient')) return c.json({ error: error.message }, 400);
    if (error.message?.includes('yourself')) return c.json({ error: error.message }, 400);
